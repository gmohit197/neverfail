...
//  Neverfail
//
//  Created by Acxiom Consulting on 15/07/20.
...
class NCMAILDELVC: BASEACTIVITY,UITextFieldDelegate {
    ...
    @IBOutlet var mailviewheight: NSLayoutConstraint!  // 300
    var mailheight = 300
    
    @IBOutlet var mainviewheight: NSLayoutConstraint!   // 1120
    var mainheight = 1120
...
//            self.showIndicator("Loading...", vc: self)
    ...
    func validate() -> Bool {
        ...
        if (self.name.text!.count == 0 || self.name.text! == " "){
            flag = false
        ...
        }else if ( self.abn.text!.count == 0 || self.abn.text! == " "){
            flag = false
            self.showToast(message: "ABN is mandatory.")
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.uid = self.getdate(format: "yyyy-MM-dd HH:mm:ss.SSS")
    ...
    @IBAction func donebtn(_ sender: Any) {
        ...
        if (self.validate()){
            ...
            if (self.checkbox.isSelected){
                self.insertnewcust(uid: uid, salutation: "", custtype: "1", fname: self.name.text!, lname: "", deladdr: self.addr.text!, delsuburb: self.suburb.text!, delstate: self.state.text!, delpostcode: self.postcode.text!,del_country : "AUS", del_lat : lati.text!, del_long : longi.text!, mail_country : "AUS", mail_lat : lati.text!, mail_long : longi.text! ,mailaddr: self.addr.text!, mailsuburb: self.suburb.text!, mailstate: self.state.text!, mailpostcode: self.postcode.text!, abn: self.abn.text!, segment: segment,subsegment: subseg, lob: lob, notes: self.notes.text)
            }else{
                self.insertnewcust(uid: uid, salutation: "", custtype: "1", fname: self.name.text!, lname: "", deladdr: self.addr.text!, delsuburb: self.suburb.text!, delstate: self.state.text!, delpostcode: self.postcode.text!,del_country : "AUS", del_lat : lati.text!, del_long : longi.text!, mail_country : "AUS", mail_lat : maillati.text!, mail_long : maillongi.text!, mailaddr: self.mailaddr.text!, mailsuburb: self.mailsuburb.text!, mailstate: self.mailstate.text!, mailpostcode: self.mailpostcode.text!, abn: self.abn.text!, segment: segment,subsegment: subseg, lob: lob, notes: self.notes.text)
            }
            let sb = UIStoryboard(name: "NEWCUSTOMER", bundle: nil)
            let vc = sb.instantiateViewController(withIdentifier: "NCCOM2VC") as! NCCONTACTVC
            vc.uid = self.uid
