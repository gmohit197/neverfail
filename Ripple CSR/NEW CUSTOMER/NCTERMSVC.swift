...
//  Neverfail
//
//  Created by Acxiom Consulting on 21/07/20.
...
class NCTERMSVC: BASEACTIVITY {
    
    @IBOutlet var checkbox: UIButton!
    
    @IBAction func checboxbtn(_ sender: UIButton) {
         animatebtn()
    }
    var uid = ""
...
//                self.showToast(message: "Internet connection required")
    ...
    override func viewDidLoad() {
        ...
        SwiftEventBus.onMainThread(self, name: "cusdone") { (result) in
            ...
            self.hideindicator()
            self.gotohome()
            self.showmsg(msg: "Commercial Customer created")
        }
