...
class SYNCDOWNLOADVC: UIViewController,UITableViewDelegate,UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return dataset.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! syncdownloadcell
        cell.tablename.text=dataset[indexPath.row]
        if indexPath.row % 3 == 0{
            cell.statusimage.image=UIImage(named: "error")
        }
        else{
            cell.statusimage.image=UIImage(named: "syncdownload")
        }
